" Settings sourced from init.lua (plugins are managed by lazy.nvim there).
" Pure vimscript: options, mappings, functions, and per-plugin configuration.
" Sourced BEFORE plugins load so g: vars take effect.
...
augroup TablineColors
  autocmd!
  autocmd ColorScheme * hi TabLine cterm=underline ctermfg=15 ctermbg=242 gui=underline guifg=White guibg=#6c6c6c
  autocmd ColorScheme * hi TabLineSel cterm=bold ctermfg=15 ctermbg=234 gui=bold guifg=White guibg=#1c1c1c
  autocmd ColorScheme * hi TabLineFill cterm=NONE ctermbg=252 gui=NONE guibg=#d0d0d0
augroup END
